...
func UnmarshalClusterName(bytes []byte, opts ...auth.MarshalOption) (types.ClusterName, error) {
...
func MarshalClusterName(c types.ClusterName, opts ...auth.MarshalOption) ([]byte, error) {
	cfg, err := CollectOptions(opts)
	if err != nil {
		return nil, trace.Wrap(err)
	}
