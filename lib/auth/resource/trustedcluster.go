...
func UnmarshalTrustedCluster(bytes []byte, opts ...auth.MarshalOption) (types.TrustedCluster, error) {
...
func MarshalTrustedCluster(c types.TrustedCluster, opts ...auth.MarshalOption) ([]byte, error) {
	cfg, err := CollectOptions(opts)
	if err != nil {
		return nil, trace.Wrap(err)
	}
