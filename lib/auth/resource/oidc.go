...
func UnmarshalOIDCConnector(bytes []byte, opts ...auth.MarshalOption) (types.OIDCConnector, error) {
...
func MarshalOIDCConnector(c types.OIDCConnector, opts ...auth.MarshalOption) ([]byte, error) {
	cfg, err := CollectOptions(opts)
	if err != nil {
		return nil, trace.Wrap(err)
	}
