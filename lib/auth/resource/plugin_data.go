...
func MarshalPluginData(data types.PluginData, opts ...auth.MarshalOption) ([]byte, error) {
...
func UnmarshalPluginData(raw []byte, opts ...auth.MarshalOption) (types.PluginData, error) {
	cfg, err := CollectOptions(opts)
	if err != nil {
		return nil, trace.Wrap(err)
	}
