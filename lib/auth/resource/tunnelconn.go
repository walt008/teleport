...
func UnmarshalTunnelConnection(data []byte, opts ...auth.MarshalOption) (types.TunnelConnection, error) {
	if len(data) == 0 {
...
func MarshalTunnelConnection(rt types.TunnelConnection, opts ...auth.MarshalOption) ([]byte, error) {
	cfg, err := CollectOptions(opts)
	if err != nil {
		return nil, trace.Wrap(err)
	}
