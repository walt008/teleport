...
func UnmarshalRemoteCluster(bytes []byte, opts ...auth.MarshalOption) (types.RemoteCluster, error) {
...
func MarshalRemoteCluster(c types.RemoteCluster, opts ...auth.MarshalOption) ([]byte, error) {
	cfg, err := CollectOptions(opts)
	if err != nil {
		return nil, trace.Wrap(err)
	}
