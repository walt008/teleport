...
func insecureRegisterClient(params RegisterParams) (*Client, error) {
	...
	client, err := New(client.Config{Addrs: utils.NetAddrsToStrings(params.Servers), TLS: tlsConfig})
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func pinRegisterClient(params RegisterParams) (*Client, error) {
	...
	tlsConfig.InsecureSkipVerify = true
	tlsConfig.Time = params.Clock.Now
	authClient, err := New(client.Config{Addrs: utils.NetAddrsToStrings(params.Servers), TLS: tlsConfig})
	if err != nil {
		return nil, trace.Wrap(err)
	}
