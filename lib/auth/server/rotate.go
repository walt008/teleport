...
func (a *Server) RotateCertAuthority(req RotateRequest) error {
	...
	for _, caType := range caTypes {
		existing, err := a.Services.Trust.GetCertAuthority(types.CertAuthID{
			Type:       caType,
		...
		if err := a.Services.Trust.CompareAndSwapCertAuthority(rotated, existing); err != nil {
			return trace.Wrap(err)
		}
...
func (a *Server) RotateExternalCertAuthority(ca types.CertAuthority) error {
	...
	if err := a.Services.Trust.CompareAndSwapCertAuthority(updated, existing); err != nil {
		return trace.Wrap(err)
	}
...
func (a *Server) autoRotate(ca types.CertAuthority) error {
	...
	if err := a.Services.Trust.CompareAndSwapCertAuthority(rotated, ca); err != nil {
		return trace.Wrap(err)
	}
